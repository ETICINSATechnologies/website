...
function wp_linkedin_autopublish_post_to_linkedin_common ($postId){
    ...
    if($options['wp_linkedin_autopublish_share_method'] == "simple"){
        ...
        $json = array(
            ...
            'owner' => 'urn:li:'.$profileType.':'.$profileId,
            'subject' => html_entity_decode(get_the_title($postId)),
            'text' => array(
                'text' => $linkedinComment
            ),
        );
        ...
        $contentEntities = array(
            'entityLocation' => get_permalink($postId),
        );
        ...
        if(strlen($thumbnailUrl)>0){
            $contentEntities['thumbnails'] = array(
                array(
        ...
        $json = array(
            'content' => array(
                'contentEntities' => array($contentEntities),
            ),
            'distribution' => array(
                'linkedInDistributionTarget' => new ArrayObject(),
            ),
    ...
    if($getCompanies !== 'ERROR'  && count($getCompanies['elements']) > 0){
        foreach($getCompanies['elements'] as $company){
            $companyNames[$company['organizationalTarget~']['id']] = $company['organizationalTarget~']['localizedName'];    
        }
    }
    ...
    foreach($profilesToShareToArray as $profile){
        ...
        if($profile == $getProfile['id']){ 
            ...
            $json['owner'] = 'urn:li:person:'.$profile;
     

        } else {
            $shareName = $companyNames[$profile];
            $json['owner'] = 'urn:li:organization:'.$profile;
        }
...
function wp_linkedin_autopublish_get_companies() {
    ...
    if ($getTransient != false){
        ...
        $companyUrl = 'https://api.linkedin.com/v2/organizationalEntityAcls?q=roleAssignee&role=ADMINISTRATOR&state=APPROVED&projection=(elements*(organizationalTarget~(id,localizedName,logoV2(original~:playableStreams))))';
...
function wp_linkedin_autopublish_get_companies_render_profile_list_items($selectedItems) {
    ...
    if($getCompanies !== "ERROR" && count($getCompanies['elements']) > 0){
        ...
        foreach ($getCompanies['elements'] as $company) {
            ...
            if(in_array($company['organizationalTarget~']['id'], $settingToArray)){

                if(in_array($company['organizationalTarget~']['id'], $selectedItems)){
                    $listClass = 'selected';
                    $iconClass = 'fa-check-circle-o';
                } else {
                    $listClass = ''; 
                ...
                $html .= '<li class="profile-selection-list-item-small '.$listClass.'" data="'.$company['organizationalTarget~']['id'].'">';

                    //image 
                    $html .= '<img src="'.$company['organizationalTarget~']['logoV2']['original~']['elements'][0]['identifiers'][0]['identifier'].'" class="location-image" height="42" width="42">';

                    //location information
                    $html .= '<div class="profile-information">';
                        
                        //address
                        $html .= '<span class="profile-name">'.$company['organizationalTarget~']['localizedName'].'</span>';
